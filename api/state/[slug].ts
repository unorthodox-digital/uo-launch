...
/**
 * Shared checklist progress for one client launch page.
 *
 *   GET  /api/state/<slug>  -> { done: {...}, interest: null }
 *   PUT  /api/state/<slug>  <- { done: {...}, interest: null }
 *
 * `interest` is the launch path the client picked. `page` - which tab they are
 * on - is deliberately NOT stored: it is a per-person view position, and
 * syncing it would move the client's tab under them when a teammate opened the
 * page. It stays in that browser's localStorage.
 *
 ...
 * nothing about it. /newpagelaunch only ever writes under
 * deploy/uo-launch/<slug>/, so a publish can never overwrite it.
 *
...
const EMPTY: PageState = { done: {}, interest: null };
...
function sanitise(input: unknown): PageState {
  ...
  const state: PageState = { done: {}, interest: null };
  ...
  if (typeof src.interest === "string") {
    state.interest = src.interest.slice(0, MAX_INTEREST_CHARS);
  }
...
async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === "GET") {
    ...
    try {
      return json(sanitise(JSON.parse(await new Response(found.stream).text())));
    } catch {
